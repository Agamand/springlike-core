...
export const Config = (path: string | string[], defaultValue?: any) => {
...
export class ConfigMgr {
    ...
    public static get(path: string | string[], defaultValue?: any): any {
        defaultValue = defaultValue || null;
        let value = null;
        if (this.isString(value)) {
            value = Utils.traverse(this.config, value);
        } else {
            for (let v of path) {
                value = Utils.traverse(this.config, v);
                if (value != undefined)
                    break;
            }
        }
